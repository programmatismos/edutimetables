...
export async function seedIfEmpty() {
  try {
    ...
    const classA = insertedClasses[0];
    const classB = insertedClasses[1];
    const classG = insertedClasses[2];
    ...
    await db.insert(schema.subjects).values([
      ...
      // Κατεύθυνση (παράδειγμα — ο χρήστης μπορεί να αλλάξει)
    ...
    // ── Εκπαιδευτικοί (τυπικές ειδικότητες ΓΕΛ) ──────────────────────────────
